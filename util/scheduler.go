...
func (y *Schedule) Stop_AllTask() error {
	log.Println("stop all task...-->begin")
	y.StopTick <- struct{}{}
	log.Println("stop all task...-->end")
	return nil
}
...
func (y *Schedule) matchTime(calendar string, t time.Time) (string, string) {
	...
	if len(times) < 6 {
		log.Println("canlendar format error!")
		return "", ""
	}
	year = times[0]
	month = times[1]
...
func (y *Schedule) Run() error {
	...
	log.Println("Blocking the Run...")
	<-y.StopTick
	log.Println("Recv a stop siganal ...")
	return nil
}
